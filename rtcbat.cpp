...
static uint8_t axpRead(uint8_t reg) {
  Wire1.beginTransmission(PMU_ADDR); Wire1.write(reg);
...
static bool axpWrite(uint8_t reg, uint8_t v) {
  Wire1.beginTransmission(PMU_ADDR); Wire1.write(reg); Wire1.write(v);
  return Wire1.endTransmission() == 0;
}
...
bool batBegin() {
  ...
  // ADC: temperatura, VBUS, batteria e VSYS, come l'example ufficiale.
  axpClrBit(0x30, 1); // TS pin measurement off
  ...
  // 4.2 V charge target (driver Waveshare: option 3).
  uint8_t cv = axpRead(0x64);
  axpWrite(0x64, (uint8_t)((cv & 0xF8) | 3));

  // Start with all PMU IRQs disabled and explicitly enable PWR-key short press.
  axpWrite(0x40, 0x00);
  axpWrite(0x41, 0x00);
  axpWrite(0x42, 0x00);
  axpWrite(0x48, 0xFF);
...
void pmuEnablePanel() {
  // BLDO1: 0.5V + 28*100mV = 3.3V; enable = bit4 of LDO_ONOFF_CTRL0.
...
static void refreshPower() {
  ...
  if (!pmuOk) {
    cachedPct = -1; cachedMv = 0; cachedCharging = false; cachedUsb = false;
    return;
  }
  ...
  cachedUsb = ((s2 & (1u << 3)) == 0) && vbusGood;

  bool batt = (s1 & (1u << 3)) != 0;
  cachedPct = batt ? axpRead(0xA4) : -1;
  // Battery ADC is H5:L8: bits 4:0 from 0x34 followed by 0x35.
  uint16_t raw = (uint16_t)((axpRead(0x34) & 0x1F) << 8) | axpRead(0x35);
...
void pwrSetup() {
  if (!pmuOk) return;
  // OFFLEVEL=00 -> hardware shutdown after 4s; keep the power-key IRQ enabled.
  uint8_t r27 = axpRead(0x27);
  r27 &= (uint8_t)~0x0C;
  axpWrite(0x27, r27);
  axpWrite(0x40, 0x00);
  axpWrite(0x41, (uint8_t)(1u << 3));
  axpWrite(0x42, 0x00);
  axpWrite(0x48, 0xFF);
  axpWrite(0x49, 0xFF);
  axpWrite(0x4A, 0xFF);
